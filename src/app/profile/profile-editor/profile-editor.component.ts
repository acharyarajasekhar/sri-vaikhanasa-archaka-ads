...
export class ProfileEditorComponent implements OnInit {
  ...
  private initDynamicFormControls() {
    return new Promise((res) => {
      ...
      this.formDataTranslationService
        .setFormConfigWithTranslatedText(this.formConfig)
        .then((translatedFormConfig: any) => {
          if (!!translatedFormConfig) {
            this.zone.run(() => {
              this.controls = this.controlSvc.getControls(
                translatedFormConfig.controls
              );
            });
          }
          res();
  ...
  onSave() {
    if (this.form.invalid) {
      if (!!this.dynamicForm) {
        ...
        console.log("TODO: How dynamic form is null or undefined...");
      }
    } else {
